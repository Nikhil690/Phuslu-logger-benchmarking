...
// levelToColor maps a log level to an ANSI color and label
...
// Custom Console Formatter with Message Divider
func customConsoleFormatter(w io.Writer, args *log.FormatterArgs) (int, error) {
	colorCode, levelLabel := levelToColor(args.Level)

	// Collect main message
	messages := []string{args.Message}

	// Extract additional key-value pairs and append them as extra messages
	for i := len(args.KeyValues) - 1; i >= 0; i-- {
		messages = append([]string{fmt.Sprintf("%v", args.KeyValues[i].Value)}, messages...)
	}

	// Join all messages with a divider " || "
	finalMessage := strings.Join(messages, " | ")

	// Format: "timestamp | colored-level | message1 || message2 || message3"
	return fmt.Fprintf(w, "%s | \033[0m%s%s\033[0m | %s\n",
		args.Time,    // e.g. "2025-03-10T13:26:02.289Z"
		colorCode,    // e.g. "\033[32m"
		levelLabel,   // e.g. "INFO "
		finalMessage, // "Main message || ExtraField: Value"
	)
}
